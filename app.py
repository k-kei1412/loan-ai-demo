...
st.title("🏦 ローンデフォルト予測 AIシステム [究極版・修正済]")
...
if submit:
    ...
    try:
        ...
        with m_col1:
            ...
            if proba > 0.05: st.error("AI判定: 🚨 危険")
            elif proba > 0.01: st.warning("AI判定: ⚠️ 注意")
            else: st.success("AI判定: ✅ 良好")

        # --- 精密類似事例検索 (エラー対策版) ---
        if not train_df.empty:
            # 検索に使う数値列（NaicsSectorは文字列が混じることがあるので除外するか強制変換）
            search_features = ["GrossApproval", "InitialInterestRate", "TermInMonths", "JobsSupported"]
            
            # 過去データから数値列だけを抽出し、文字列を強制的にNaNにしてから埋める
            train_sub = train_df[search_features].copy()
            for col in search_features:
                train_sub[col] = pd.to_numeric(train_sub[col], errors='coerce')
            train_sub = train_sub.fillna(train_sub.mean())

            input_sub = input_df[search_features].copy()
            for col in search_features:
                input_sub[col] = pd.to_numeric(input_sub[col], errors='coerce')
            ...
            if 'Default' in similar_cases.columns:
                ...
                with m_col2:
                    ...
                    if actual_risk_pct >= 20.0: st.error("実績判定: 🚨 危険")
                    elif actual_risk_pct > 0: st.warning("実績判定: ⚠️ 要注意")
                    else: st.success("実績判定: ✨ 非常に安全")

                with m_col3:
                    diff = actual_risk_pct - (proba * 100)
                    if diff > 10: st.error("信頼性警告: AIが楽観的すぎます")
                    else: st.success("信頼性: 正常")
