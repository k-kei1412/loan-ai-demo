...
if submit:
    ...
    try:
        ...
        with col_tips:
            ...
            # 実績事故率(risk_pct)に応じてメッセージを分岐させる
            if risk_pct > 50:
                st.error(f"🚨 【極めて危険】類似事例の半数以上（{risk_pct:.1f}%）がデフォルトしています。AIの確信度にかかわらず、本案件の承認は極めて慎重に行うべきです。")
            elif risk_pct > 20:
                st.error(f"⚠️ 【高リスク】類似事例の {risk_pct:.1f}% で事故が発生しています。過去の失敗事例（赤色の行）と共通する要因がないか、徹底的に精査してください。")
            elif def_count > 0:
                st.warning(f"💡 【注意】デフォルトが {def_count} 件確認されました。AIは期間の長さを警戒していますが、実績事故率が低い（{risk_pct:.1f}%）ため、他の好材料があれば検討の余地があります。")
            else:
                st.success("✅ 【良好】類似事例にデフォルトは見当たりません。AIの確信度も高く、データ上は非常にクリーンな案件です。")
